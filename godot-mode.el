;; I just replaced some shit in python-mode and called it gd/godot
;; also added "func" and "var" at some places where there was previously just "def"
;;--------------------------------------------------------------------------------
...
;; Author: 2003-2013 https://launchpad.net/godot-mode
;;         1995-2002 Barry A. Warsaw
;;         1992-1994 Tim Peters
;; Maintainer: godot-mode@python.org
;; Created:    Feb 1992
...
;; commands-godot-mode.org in directory "doc" reports
;; available commands, also a menu is provided

;; as for `gd-add-abbrev':
;; Similar to `add-mode-abbrev', but uses
;; `gd-partial-expression' before point for expansion to
...
;; `gd-expression' composed by three
;; gd-partial-expressions.
;;
;; OrderedDict.popitem(last=True)
;;
;; Putting the curser at the EOL, M-3 M-x gd-add-abbrev
;;
;; would prompt "op" for an abbrev to store, as first
;; `gd-partial-expression' beginns with a "(", which is
...
(defconst gd-version "6.2.0+")

(defcustom gd-install-directory ""
  "Directory where godot-mode.el and it's subdirectories should be installed. Needed for completion and other environment stuff only. "
...
(defcustom godot-mode-modeline-display "Py"
  "String to display in Emacs modeline "

  :type 'string
  :tag "godot-mode-modeline-display"
  :group 'godot-mode)

(defcustom gd-extensions "gd-extensions.el"
  "File where extensions to godot-mode.el should be installed. Used by virtualenv support. "
...
(defcustom gd-shell-unfontify-p t
  "Run `gd--run-unfontify-timer' unfontifying the shell banner-text.
...
(defvar gd--shell-unfontify nil
  "Internally set. ")
(make-variable-buffer-local 'gd--shell-unfontify)
...
Restart gd-shell once with new Emacs/godot-mode. ")
...
(defcustom gd-auto-fill-mode nil
  "If godot-mode should set fill-column
...
(defvar gd-python-completions "*Python Completions*"
...
(defcustom gd-smart-operator-mode-p nil
  "If godot-mode calls `smart-operator-mode-on'
...
(defcustom gd-autopair-mode nil
  "If godot-mode calls (autopair-mode-on)
...
(defcustom gd-auto-complete-p nil
  "Run godot-mode's built-in auto-completion via gd-complete-function. Default is  nil"
...
(defcustom gd-set-complete-keymap-p  nil
  "If `gd-complete-initialize', which sets up enviroment for Pymacs based gd-complete, should load it's keys into `godot-mode-map'
...
(defcustom gd-indent-paren-spanned-multilines-p nil
...
(defcustom gd-imenu-max-items 99
 "godot-mode specific `imenu-max-items'"
...
(defcustom gd-tab-indent t
  "Non-nil means TAB in Python mode calls `gd-indent-line'."
  :type 'boolean
  :tag "gd-tab-indent"
  :group 'godot-mode)
...
Normally godot-mode knows best which function to use. "
  :type '(choice
...
If empty, godot-mode will guess some "
  :type 'string
...
(defcustom gd-indent-tabs-mode nil
  "godot-mode starts `indent-tabs-mode' with the value specified here, default is nil. "
  :type 'boolean
  :tag "gd-indent-tabs-mode"
  :group 'godot-mode)

(defcustom gd-smart-indentation t
  "Should `godot-mode' try to automagically set some indentation variables?
...
to `godot-mode':
 ...
 Note that both these settings occur *after* `godot-mode-hook' is run,
 so if you want to defeat the automagic configuration, you must also
 set `gd-smart-indentation' to nil in your `godot-mode-hook'."
  :type 'boolean
...
(defvar gd-python-ms-pdb-command ""
...
When `godot-mode' tries to calculate the shell to use (either a
...
(defcustom gd-current-defun-show t
  "If `gd-current-defun' should jump to the definition, highlight it while waiting GD-WHICH-FUNC-DELAY seconds, before returning to previous position.
...
(defcustom gd-current-defun-delay 2
  "When called interactively, `gd-current-defun' should wait GD-WHICH-FUNC-DELAY seconds at the definition name found, before returning to previous position. "

  :type 'number
  :tag "gd-current-defun-delay"
  :group 'godot-mode)

(defcustom gd-python-send-delay 5
  "Seconds to wait for output, used by `gd--send-...' functions.

See also gd-ipython-send-delay"

  :type 'number
  :tag "gd-python-send-delay"
  :group 'godot-mode)

(defcustom gd-ipython-send-delay 9
  "Seconds to wait for output, used by `gd--send-...' functions.

See also gd-python-send-delay"
...
Otherwise value of gd-python-history is used. "
  :type 'boolean
  :tag "gd-honor-PYTHONHISTORY-p"
  :group 'godot-mode)

(defcustom gd-python-history "~/.python_history"
  "python-history default file. Used when gd-honor-PYTHONHISTORY-p is nil (default) "

  :type 'string
  :tag "gd-python-history"
  :group 'godot-mode)
...
When set to `t', godot-mode tries to reuse existing windows and will split only if needed.
...
(defcustom gd-hide-show-keywords
  '("class"    "var"    "func"    "elif"    "else"    "except"
...
Else /usr/bin/python"
  ...
  :type 'string
  :tag "gd-python-command
"
  :group 'godot-mode)

(defcustom gd-python-command-args "-i"
  "String arguments to be used when starting a Python shell."
  :type 'string
  :tag "gd-python-command-args"
  :group 'godot-mode)
...
Sets `gd-match-paren-key' in godot-mode-map.
...
Making switch between several virtualenv's easier,
                               `godot-mode' should deliver an installer, so named-shells pointing to virtualenv's will be available. "
  :type 'boolean
  :tag "gd-use-local-default"
  :group 'godot-mode)

(defcustom gd-edit-only-p nil
  "When `t' `godot-mode' will not take resort nor check for installed Python executables. Default is nil.
...
(defcustom gd-force-gd-shell-name-p nil
  "When `t', execution with kind of Python specified in `gd-shell-name' is enforced, possibly shebang doesn't take precedence. "

  :type 'boolean
  :tag "gd-force-gd-shell-name-p"
  :group 'godot-mode)

(defcustom godot-mode-v5-behavior-p nil
  "Execute region through `shell-command-on-region' as
v5 did it - lp:990079. This might fail with certain chars - see UnicodeEncodeError lp:550661"

  :type 'boolean
  :tag "godot-mode-v5-behavior-p"
  :group 'godot-mode)

(defcustom gd-trailing-whitespace-smart-delete-p nil
  "Default is nil. When t, godot-mode calls
...
(defcustom gd--warn-tmp-files-left-p nil
  "Messages a warning, when `gd-temp-directory' contains files susceptible being left by previous godot-mode sessions. See also lp:987534 "
  :type 'boolean
...
(defcustom gd-python-edit-version ""
...
(defvaralias 'inferior-python-filter-regexp 'gd-input-filter-re)
...
Used by Python-shell for output of `gd-execute-buffer' and related commands. See also `gd-use-current-dir-when-execute-p'"
  :type 'string
  :tag "gd-execute-directory"
  :group 'godot-mode)

(defcustom gd-use-current-dir-when-execute-p t
  "When `t', current directory is used by Python-shell for output of `gd-execute-buffer' and related commands.
...
(defcustom gd-fileless-buffer-use-default-directory-p t
  "When `gd-use-current-dir-when-execute-p' is non-nil and no buffer-file exists, value of `default-directory' sets current working directory of Python output shell"
  :type 'boolean
...
(defvar gd-this-abbrevs-changed nil
  "Internally used by godot-mode-hook")
...
(defun gd--set-ffap-form ()
  (cond ((and gd-ffap-p gd-ffap)
         (eval-after-load "ffap"
           '(push '(godot-mode . gd-module-path) ffap-alist))
         (setq ffap-alist (remove '(godot-mode . gd-ffap-module-path) ffap-alist))
         (setq ffap-alist (remove '(gd-shell-mode . gd-ffap-module-path)
                                  ffap-alist)))
        (t (setq ffap-alist (remove '(godot-mode . gd-ffap-module-path) ffap-alist))
           (setq ffap-alist (remove '(gd-shell-mode . gd-ffap-module-path)
                                    ffap-alist))
           (setq ffap-alist (remove '(godot-mode . gd-module-path) ffap-alist)))))

(defcustom gd-ffap-p nil

  "Select godot-modes way to find file at point.
...
(defvar gd-output-buffer "*Python Output*"
    "Currently unused.
...
(defvar godot-mode-message-string "python-components-mode.el"
  "Internally used. Reports the godot-mode branch in use.")
...
(defvar godot-mode-syntax-table nil
  "Give punctuation syntax to ASCII that normally has symbol
syntax or has word syntax and isn't a letter.")

(setq godot-mode-syntax-table
...
(defvar gd-ipython0.10-completion-command-string
  "print(';'.join(__IP.Completer.all_completions('%s'))) #GODOT-MODE SILENT\n"
  "The string send to ipython to query for all possible completions")

(defvar gd-ipython0.11-completion-command-string
  "print(';'.join(get_ipython().Completer.all_completions('%s'))) #GODOT-MODE SILENT\n"
  "The string send to ipython to query for all possible completions")
...
(defvar gd-which-bufname "Python")
...
(defvar gd-completion-last-window-configuration nil
...
(defvar gd-dotted-expression-syntax-table
  (let ((table (make-syntax-table godot-mode-syntax-table)))
    (modify-syntax-entry ?_ "_" table)
...
(defconst gd-no-outdent-1-re-raw
  (list
   "elif"
   "else"
   "except"
...
(defconst gd-no-outdent-2-re-raw
  (list
   "break"
   "continue"
   "pass"
...
(defconst gd-no-outdent-re
  (concat
   "[ \t]*\\_<\\("
   (regexp-opt gd-no-outdent-1-re-raw)
   "\\)\\_>.*:[( \t]\\_<\\("
   (regexp-opt gd-no-outdent-2-re-raw)
   "\\)\\_>[)\t]*$")
  "Regular expression matching lines not to augment indent after.

See gd-no-outdent-1-re-raw, gd-no-outdent-2-re-raw for better readable content ")

(defconst gd-assignment-re "\\_<\\w+\\_>[ \t]*\\(=\\|+=\\|*=\\|%=\\|&=\\|^=\\|<<=\\|-=\\|/=\\|**=\\||=\\|>>=\\|//=\\)"
  "If looking at the beginning of an assignment. ")

(defconst gd-block-re "[ \t]*\\_<\\(class\\|def\\|for\\|if\\|try\\|while\\|with\\)\\_>[:( \n\t]*"
  "Matches the beginning of a compound statement. ")

(defconst gd-minor-block-re "[ \t]*\\_<\\(for\\|if\\|try\\|with\\|except\\)\\_>[:( \n\t]*"
...
(defconst gd-for-block-re "[ \t]*\\_<for\\_> +[[:alpha:]_][[:alnum:]_]* +in +[[:alpha:]_][[:alnum:]_()]* *[: \n\t]"
  "Matches the beginning of a `try' block. ")
...
(defconst gd-def-or-class-re "[ \t]*\\_<\\(def\\|class\\)\\_>[ \n\t]"
  "Matches the beginning of a class- or functions definition. ")

(defconst gd-def-re "[ \t]*\\_<\\(def\\)\\_>[ \n\t]"
  "Matches the beginning of a functions definition. ")

(defconst gd-block-or-clause-re-raw
  (list
   "elif"
   "else"
   "except"
...
(defconst gd-extended-block-or-clause-re-raw
  (list
   "class"
   "var"
   "func"
...
(defun gd--python-send-setup-code-intern (name)
  (let ((setup-file (concat (gd--normalize-directory gd-temp-directory) "gd-" name "-setup-code.py"))
    (gd-ignore-result-p t))
    (unless (file-readable-p setup-file)
      (with-temp-buffer
    (insert (eval (car (read-from-string (concat "gd-" name "-setup-code")))))
    (write-file setup-file)))
    (gd--execute-file-base nil setup-file nil (current-buffer))))

(defun gd--python-send-completion-setup-code ()
  "For Python see gd--python-send-setup-code "
  (gd--python-send-setup-code-intern "shell-completion"))

(defun gd--python-send-ffap-setup-code ()
  "For Python see gd--python-send-setup-code "
  (gd--python-send-setup-code-intern "ffap"))

(defun gd--python-send-eldoc-setup-code ()
  "For Python see gd--python-send-setup-code "
  (gd--python-send-setup-code-intern "eldoc"))
...
(and (fboundp 'make-obsolete-variable)
     (make-obsolete-variable 'gd-mode-hook 'godot-mode-hook nil))
...
(defun gd--choose-shell-by-import ()
...
(defun gd-python-current-environment ()
...
(defun gd--cleanup-process-name (res)
...
This does the following:
 ...
 - examine imports using `gd--choose-shell-by-import'
  ...
  (if (eq 4 (prefix-numeric-value arg))
    ...
    (let* (res done
      ...
      (if cmd
          (when (called-interactively-p 'any)
            (message "%s" cmd))
        (when (called-interactively-p 'any) (message "%s" "Could not detect Python on your system. Maybe set `gd-edit-only-p'?")))
      erg)))
...
if `(locate-library \"godot-mode\")' is not succesful.

Used only, if `gd-install-directory' is empty. "
  (interactive)
  (let ((erg (cond ((locate-library "godot-mode")
                    (file-name-directory (locate-library "godot-mode")))
                   ((and (buffer-file-name)(string-match "godot-mode" (buffer-file-name)))
                    (file-name-directory (buffer-file-name)))
                   ((string-match "godot-mode" (buffer-name))
                    default-directory))))
...
(defun gd-load-pymacs ()
  "Load Pymacs as delivered with godot-mode.el.
...
See original source: http://pymacs.progiciels-bpi.ca"
  ...
  (let ((pyshell (gd-choose-shell))
        ...
        (gd-install-directory (cond ((string= "" gd-install-directory)
                                    ...
                                    (t (gd--normalize-directory gd-install-directory)))))
    (if (gd-install-directory-check)
        (progn
          ;; If Pymacs has not been loaded before, prepend gd-install-directory to
          ;; PYTHONPATH, so that the Pymacs delivered with godot-mode is used.
...
(when (or gd-load-pymacs-p (featurep 'pymacs))
    ...
    (let* ((path (getenv "PYTHONPATH"))
      ...
      (if (gd-install-directory-check)
          (progn
            ...
            (require 'pycomplete)
            (add-hook 'godot-mode-hook 'gd-complete-initialize))
        (error "`gd-install-directory' not set, see INSTALL")))))

(and (or (eq gd-complete-function 'gd-complete-completion-at-point) gd-load-pymacs-p (featurep 'pymacs))
  (gd-load-pycomplete))

(defun gd-set-load-path ()
  "Include needed subdirs of godot-mode directory. "
  (interactive)
  (let ((gd-install-directory (gd--normalize-directory gd-install-directory)))
...
(setq godot-mode-map
      (let ((map (make-sparse-keymap)))
        ...
        ;; Miscellaneous
        ;; (define-key map [(super q)] 'gd-cogd-statement)
...
(eval-and-compile
  (defconst python-rx-constituents
    `((block-start . ,(rx symbol-start
              (or "var" "func" "class" "if" "elif" "else" "try"
...
(setq python-font-lock-keywords
      ...
      `(,(rx symbol-start
          ...
          "return"  "for" "lambda")
             symbol-end)
        (,(rx symbol-start (or "def" "class") symbol-end) . gd-def-class-face)
...
;;  godot-mode-v5-behavior-p forms
(defun toggle-godot-mode-v5-behavior-p (&optional arg)
  "If `godot-mode-v5-behavior-p' should be on or off.

  Returns value of `godot-mode-v5-behavior-p' switched to. "
  (interactive)
  (let ((arg (or arg (if godot-mode-v5-behavior-p -1 1))))
    (if (< 0 arg)
        (setq godot-mode-v5-behavior-p t)
      (setq godot-mode-v5-behavior-p nil))
    (when (or gd-verbose-p (called-interactively-p 'any)) (message "godot-mode-v5-behavior-p: %s" godot-mode-v5-behavior-p))
    godot-mode-v5-behavior-p))

(defun godot-mode-v5-behavior-p-on (&optional arg)
  "Make sure, `godot-mode-v5-behavior-p' is on.

Returns value of `godot-mode-v5-behavior-p'. "
  (interactive)
  (let ((arg (or arg 1)))
    (toggle-godot-mode-v5-behavior-p arg))
  (when (or gd-verbose-p (called-interactively-p 'any)) (message "godot-mode-v5-behavior-p: %s" godot-mode-v5-behavior-p))
  godot-mode-v5-behavior-p)

(defun godot-mode-v5-behavior-p-off ()
  "Make sure, `godot-mode-v5-behavior-p' is off.

Returns value of `godot-mode-v5-behavior-p'. "
  (interactive)
  (toggle-godot-mode-v5-behavior-p -1)
  (when (or gd-verbose-p (called-interactively-p 'any)) (message "godot-mode-v5-behavior-p: %s" godot-mode-v5-behavior-p))
  godot-mode-v5-behavior-p)
  ...
  (let ((arg (or arg (if gd-underscore-word-syntax-p -1 1))))
    (if (< 0 arg)
        (progn
          ...
          (modify-syntax-entry ?\_ "w" godot-mode-syntax-table))
      (setq gd-underscore-word-syntax-p nil)
      (modify-syntax-entry ?\_ "_" godot-mode-syntax-table))
  ...
  (let ((orig (copy-marker (point)))
    ...
    (setq need
      (cond ((bolp)
        ...
        (t (gd--calculate-indent-backwards cui this-indent-offset))))
...
(defun gd--delete-trailing-whitespace (orig)
  ...
  (when (or gd-newline-delete-trailing-whitespace-p gd-trailing-whitespace-smart-delete-p)
    ...
    (goto-char orig)
    (if (empty-line-p)
        (if (gd---emacs-version-greater-23)
...
"
  ...
  (let ((orig (copy-marker (point)))
    ...
    (gd--indent-line-by-line beg end)
      ;; (unless (empty-line-p) (gd-indent-line nil t))
...
(defalias 'gd-cogd-declarations 'gd-declarations)
...
(defalias 'gd-cogd-statements 'gd-statements)
(defun gd-statements ()
...
(defun gd-backward-class (&optional indent)
...
When `gd-mark-decorators' is non-nil, decorators are considered too. "
  (interactive)
  (gd--backward-prepare indent 'gd-class-re 'gd-extended-block-or-clause-re (called-interactively-p 'any)))
...
(defun gd-backward-def (&optional indent)
...
When `gd-mark-decorators' is non-nil, decorators are considered too. "
  (interactive)
  (gd--backward-prepare indent 'gd-def-re 'gd-extended-block-or-clause-re (called-interactively-p 'any)))

(defun gd-backward-def-or-class (&optional indent)
...
When `gd-mark-decorators' is non-nil, decorators are considered too. "
  (interactive)
  (gd--backward-prepare indent 'gd-def-or-class-re 'gd-extended-block-or-clause-re (called-interactively-p 'any)))
...
(defun gd-backward-class-bol (&optional indent)
...
When `gd-mark-decorators' is non-nil, decorators are considered too. "
  (interactive)
  (gd--backward-prepare indent 'gd-class-re 'gd-extended-block-or-clause-re (called-interactively-p 'any) t))
...
(defun gd-backward-def-bol (&optional indent)
...
When `gd-mark-decorators' is non-nil, decorators are considered too. "
  (interactive)
  (gd--backward-prepare indent 'gd-def-re 'gd-extended-block-or-clause-re (called-interactively-p 'any) t))

(defun gd-backward-def-or-class-bol (&optional indent)
...
When `gd-mark-decorators' is non-nil, decorators are considered too. "
  (interactive)
  (gd--backward-prepare indent 'gd-def-or-class-re 'gd-extended-block-or-clause-re (called-interactively-p 'any) t))
...
Returns end of try-block if successful, nil otherwise"
  (interactive "P")
  (let* ((orig (point))
         (erg (gd--end-base 'gd-try-block-re orig)))
    (when (and gd-verbose-p (called-interactively-p 'any)) (message "%s" erg))
    erg))
...
;;  Statement
(defun gd-backward-statement (&optional orig done limit)
    ...
    (unless done
      (and (< 0 (abs (skip-chars-backward " \t\r\n\f")))
        ...
        (cond
         ((and (bolp)(eolp))
          (skip-chars-backward " \t\r\n\f")
          (gd-backward-statement orig done limit))
         ((nth 8 pps)
      ;; inside string
...
See also `gd-up-statement': up from current definition to next beginning of statement above. "
  (interactive)
  (let* ((indent (or indent (when (eq 'gd-forward-statement-bol (car
  gd-bol-forms-last-indent))(cdr gd-bol-forms-last-indent))))
     (orig (point))
         erg)
    (if indent
        (while (and (setq erg (gd-backward-statement)) (< indent (current-indentation))(not (bobp))))
      (setq erg (gd-backward-statement)))
    ;; reset
    (setq gd-bol-forms-last-indent nil)
    (beginning-of-line)
...
(defalias 'beginning-of-class 'gd-beginning-of-class)
(defalias 'end-of-def-or-class 'gd-end-of-def-or-class)
...
(defalias 'gd-previous-block 'gd-backward-block)
...
Stores data in kill ring"
  (interactive "*")
  (let ((erg (gd--mark-base "elif-block")))
    (kill-region (car erg) (cdr erg))))
...
Stores data in kill ring"
  (interactive "*")
  (let ((erg (gd--mark-base "else-block")))
    (kill-region (car erg) (cdr erg))))
...
Stores data in kill ring"
  (interactive "*")
  (let ((erg (gd--mark-base "except-block")))
    (kill-region (car erg) (cdr erg))))
...
Stores data in kill ring"
  (interactive "*")
  (let ((erg (gd--mark-base "for-block")))
    (kill-region (car erg) (cdr erg))))
...
Stores data in kill ring"
  (interactive "*")
  (let ((erg (gd--mark-base "try-block")))
    (kill-region (car erg) (cdr erg))))

(defun gd-kill-block-bol ()
...
(defun gd-kill-block-or-clause-bol ()
...
(defun gd-kill-def-or-class-bol ()
...
Returns beginning and end positions of marked area, a cons. "
  (interactive)
  (let (erg)
    (setq erg (gd--mark-base "block-or-clause"))
    (exchange-point-and-mark)
    (when (and gd-verbose-p (called-interactively-p 'any)) (message "%s" erg))
    erg))
...
Returns beginning and end positions of marked area, a cons. "
  ...
  (let ((gd-mark-decorators (or arg gd-mark-decorators))
        erg)
    (gd--mark-base "class" gd-mark-decorators)
    (exchange-point-and-mark)
    (when (and gd-verbose-p (called-interactively-p 'any)) (message "%s" erg))
    erg))
...
Returns beginning and end positions of marked area, a cons. "
  ...
  (let ((gd-mark-decorators (or arg gd-mark-decorators))
        erg)
    (gd--mark-base "def" gd-mark-decorators)
    (exchange-point-and-mark)
    (when (and gd-verbose-p (called-interactively-p 'any)) (message "%s" erg))
    erg))
...
Returns beginning and end positions of marked area, a cons. "
  ...
  (let ((gd-mark-decorators (or arg gd-mark-decorators))
        erg)
    (gd--mark-base "def-or-class" gd-mark-decorators)
    (exchange-point-and-mark)
    (when (and gd-verbose-p (called-interactively-p 'any)) (message "%s" erg))
    erg))
...
Returns beginning and end positions of marked area, a cons. "
  (interactive)
  (let (erg)
    (setq erg (gd--mark-base "except-block"))
    (exchange-point-and-mark)
    (when (and gd-verbose-p (called-interactively-p 'any)) (message "%s" erg))
    erg))
...
Returns beginning and end positions of marked area, a cons. "
  (interactive)
  (let (erg)
    (setq erg (gd--mark-base "if-block"))
    (exchange-point-and-mark)
    (when (and gd-verbose-p (called-interactively-p 'any)) (message "%s" erg))
    erg))
...
Returns beginning and end positions of marked area, a cons. "
  (interactive)
  (let (erg)
    (setq erg (gd--mark-base "minor-block"))
    (exchange-point-and-mark)
    (when (and gd-verbose-p (called-interactively-p 'any)) (message "%s" erg))
    erg))
...
Returns beginning and end positions of marked area, a cons. "
  (interactive)
  (let (erg)
    (setq erg (gd--mark-base "statement"))
    (exchange-point-and-mark)
    (when (and gd-verbose-p (called-interactively-p 'any)) (message "%s" erg))
    erg))
...
Returns beginning and end positions of marked area, a cons. "
  (interactive)
  (let (erg)
    (setq erg (gd--mark-base "try-block"))
    (exchange-point-and-mark)
    (when (and gd-verbose-p (called-interactively-p 'any)) (message "%s" erg))
    erg))

(defun gd-mark-block-bol ()
...
(defun gd-mark-block-or-clause-bol ()
...
(defun gd-mark-class-bol (&optional arg)
...
(defun gd-mark-comment-bol ()
  "Mark comment, take beginning of line positions.

Returns beginning and end positions of region, a cons. "
  (interactive)
  (let (erg)
    (setq erg (gd--mark-base-bol "comment"))
    (exchange-point-and-mark)
    (when (and gd-verbose-p (called-interactively-p 'any)) (message "%s" erg))
    erg))

(defun gd-mark-def-bol (&optional arg)
...
(defun gd-mark-def-or-class-bol (&optional arg)
...
(defun gd-mark-except-block-bol ()
  "Mark except-block, take beginning of line positions.

Returns beginning and end positions of region, a cons. "
  (interactive)
  (let (erg)
    (setq erg (gd--mark-base-bol "except-block"))
    (exchange-point-and-mark)
    (when (and gd-verbose-p (called-interactively-p 'any)) (message "%s" erg))
    erg))

(defun gd-mark-expression-bol ()
  "Mark expression, take beginning of line positions.

Returns beginning and end positions of region, a cons. "
  (interactive)
  (let (erg)
    (setq erg (gd--mark-base-bol "expression"))
    (exchange-point-and-mark)
    (when (and gd-verbose-p (called-interactively-p 'any)) (message "%s" erg))
    erg))

(defun gd-mark-if-block-bol ()
...
(defun gd-mark-line-bol ()
  "Mark line, take beginning of line positions.

Returns beginning and end positions of region, a cons. "
  (interactive)
  (let (erg)
    (setq erg (gd--mark-base-bol "line"))
    (exchange-point-and-mark)
    (when (and gd-verbose-p (called-interactively-p 'any)) (message "%s" erg))
    erg))

(defun gd-mark-minor-block-bol ()
...
(defun gd-mark-paragraph-bol ()
  "Mark paragraph, take beginning of line positions.

Returns beginning and end positions of region, a cons. "
  (interactive)
  (let (erg)
    (setq erg (gd--mark-base-bol "paragraph"))
    (exchange-point-and-mark)
    (when (and gd-verbose-p (called-interactively-p 'any)) (message "%s" erg))
    erg))

(defun gd-mark-partial-expression-bol ()
  "Mark partial-expression, take beginning of line positions.

Returns beginning and end positions of region, a cons. "
  (interactive)
  (let (erg)
    (setq erg (gd--mark-base-bol "partial-expression"))
    (exchange-point-and-mark)
    (when (and gd-verbose-p (called-interactively-p 'any)) (message "%s" erg))
    erg))

(defun gd-mark-statement-bol ()
...
(defun gd-mark-try-block-bol ()
  "Mark try-block, take beginning of line positions.

Returns beginning and end positions of region, a cons. "
  (interactive)
  (let (erg)
    (setq erg (gd--mark-base-bol "try-block"))
    (exchange-point-and-mark)
    (when (and gd-verbose-p (called-interactively-p 'any)) (message "%s" erg))
    erg))

;; python-components-cogd-forms
...
Store data in kill ring, so it might yanked back. "
  (interactive "*")
  (let ((erg (gd--mark-base "block")))
    (cogd-region-as-kill (car erg) (cdr erg))))

(defun gd-cogd-block-or-clause ()
...
(defun gd-cogd-partial-expression ()
...
(defun gd-cogd-block-or-clause-bol ()
  "Delete block-or-clause bol at point.
...
(defun gd-cogd-def-or-class-bol ()
  "Delete def-or-class bol at point.
...
(defun gd-cogd-partial-expression-bol ()
...
(defun gd-delete-block-or-clause-bol ()
...
(defun gd-delete-class-bol (&optional arg)
...
(defun gd-delete-def-bol (&optional arg)
...
(defun gd-delete-def-or-class-bol (&optional arg)
...
(defun gd-delete-elif-block-bol ()
...
(defun gd-delete-else-block-bol ()
...
(defun gd-delete-except-block-bol ()
...
(defun gd-delete-minor-block-bol ()
...
`gd-choose-shell' for command and options to pass to the Python
interpreter.
"
  (interactive "P")
...
When code is executed and `gd-split-window-on-execute' is `t', the result is displays in an output-buffer, \"\*Python\*\" by default.
...
(defun gd--shell-manage-windows (output-buffer windows-config &optional exception-buffer)
  ...
  (let* ((oldbuf (current-buffer))
     (gd-exception-buffer (or exception-buffer (and gd-exception-buffer (buffer-live-p gd-exception-buffer) gd-exception-buffer)))
    ...
    ;; gd-ert-always-reuse-lp-1361531-test would fail with
    ;; save-excursion form
    (display-buffer output-buffer)
    ;;)
...
(defun gd-kill-default-shell-unconditional ()
  ...
  (interactive)
  (gd-kill-buffer-unconditional "*Python*"))
...
(defun gd--provide-command-args (fast-process argprompt)
  (cond (fast-process nil)
    ((eq 2 (prefix-numeric-value argprompt))
     (read-string "Gd-Shell arguments: "
              gd-python-command-args))
    ((string-match "^[Ii]" gd-shell-name)
     gd-ipython-command-args)
    ((string-match "^[^-]+3" gd-shell-name)
     gd-python3-command-args)
    (t gd-python-command-args)))
...
(defun gd-shell (&optional argprompt dedicated shell buffer-name fast-process exception-buffer)
  ...
  \\[universal-argument] 2 prompts for `gd-python-command-args'.
  ...
  (let* ((iact (or (called-interactively-p 'any) (eq 1 argprompt))) ;; interactively?
    ...
    (gd--create-new-shell executable args exception-buffer))
      (when (or (called-interactively-p 'any)
        ...
        argprompt
        gd-switch-buffers-on-execute-p gd-split-window-on-execute)
...
Per default it's \"(format \"execfile(r'%s') # GODOT-MODE\\n\" filename)\" for Python 2 series."
  (interactive)
  (let* ((erg (gd-which-python))
         (cmd (if (< erg 3)
                  (format "execfile(r'%s') # GODOT-MODE\n" filename)
                (format "exec(compile(open('%s').read(), '%s', 'exec')) # GODOT-MODE\n" filename filename))))
...
(defun gd--close-execution (tempbuf tempfile)
  ...
  (unless gd-debug-p
    (gd-kill-buffer-unconditional tempbuf)
    (gd-delete-temporary tempfile tempbuf)))
...
(defun gd--execute-base (&optional start end shell filename proc file wholebuf)
  ...
  (let* ((gd-exception-buffer (or gd-exception-buffer (current-buffer)))
     ...
     (proc (cond (proc)
             ...
             (gd-fast-process-p (get-buffer-process (gd-fast-process buffer)))
...
When optional FILE is `t', no temporary file is needed. "
  (let (output-buffer erg)
    ...
    (cond (gd-fast-process-p (gd--send-to-fast-process strg proc output-buffer))
      ;; enforce proceeding as godot-mode.el v5
      (godot-mode-v5-behavior-p
       (gd-execute-godot-mode-v5 start end gd-exception-buffer origline))
      ...
      (t (gd--execute-buffer-finally strg execute-directory wholebuf which-shell proc buffer origline)))))
...
(defun gd--execute-buffer-finally (strg execute-directory wholebuf which-shell proc procbuf origline)
  (let* ((temp (make-temp-name
        ;; FixMe: that should be simpler
         ...
         (tempfile (concat (expand-file-name gd-temp-directory) gd-separator-char (replace-regexp-in-string gd-separator-char "-" temp) ".py"))
    ...
    (with-current-buffer tempbuf
      ...
      (write-file tempfile))
    (unwind-protect
    (setq erg (gd--execute-file-base proc tempfile nil procbuf gd-orig-buffer-or-file nil execute-directory gd-exception-buffer origline)))
    (sit-for 0.1 t)
...
(defun gd--postprocess-comint (output-buffer origline windows-config gd-exception-buffer orig)
  ...
  (let (gd-result gd-result-raw gd-error)
    ...
    ;; gd-ert-wrong-python-test fails otherwise
...
May we get rid of the temporary file? "
  ...
  (let* ((start (copy-marker start))
         ...
         (tempfile (or (buffer-file-name) (concat (expand-file-name gd-temp-directory) gd-separator-char (replace-regexp-in-string gd-separator-char "-" "temp") ".py")))
...
(defun gd-execute-godot-mode-v5 (start end &optional gd-exception-buffer origline)
  (interactive "r")
  (let ((gd-exception-buffer (or gd-exception-buffer (current-buffer)))
...
Returns position where output starts. "
  ...
  (let* ((origline (or (ignore-errors origline) 1))
     (cmd (or cmd (format "exec(compile(open('%s').read(), '%s', 'exec')) # GODOT-MODE\n" filename filename)))
...
;;                       (point))))
;;       (gd-execute-region beg (line-end-position)))))
...
stack."
  ...
  (let* ((proc (get-process "Python"))
         (buffer (if proc "*Python*" gd-output-buffer)))
    (if bottom
...
(defun gd--after-change-function (beg end len)
  ...
  (when
    ...
    (set-window-configuration
     gd-completion-last-window-configuration))
  (goto-char end))
...
(defun gd--try-completion (input completion)
  ...
  (let (erg newlist)
    ...
    (unless (string= erg elt)
      ...
      (if (< 1 (length newlist))
      (with-output-to-temp-buffer gd-python-completions
...
(defun gd-shell-complete (&optional shell debug beg end word)
  ...
  (save-excursion
    (and (buffer-live-p (get-buffer "*Python Completions*"))
     (gd-kill-buffer-unconditional "*Python Completions*")))
  (setq gd-completion-last-window-configuration
...
In godot-mode `gd-complete-function' is called,
in (I)Python shell-modes `gd-shell-complete'"
  (interactive "*")
...
recently visited godot-mode buffer which either has the name of the
current function \(or class) or which defines the function \(or
class).  This is to provide for remote scripts, eg, Zope's 'Script
\(Python)' - put a _copy_ of the script in a buffer named for the
script, and set to godot-mode, and pdbtrack will find it.)"
...
Failing that, we look for the most recently visited godot-mode buffer
...
We walk the buffer-list history for godot-mode buffers that are
...
(defun gd-execute-statement-pdb ()
  ...
  (interactive)
  (let ((gd-python-command-args "-i -m pdb"))
...
At GNU Linux systems required pdb version should be detected by `gd--pdb-version', at Windows configure `gd-python-ms-pdb-command'

lp:963253"
  (interactive
   (list (gud-query-cmdline
      (if (or (eq system-type 'ms-dos)(eq system-type 'windows-nt))
          (car (read-from-string gd-python-ms-pdb-command))
        ;; sys.version_info[0]
        (car (read-from-string (gd--pdb-version)))) "asdf")))
...
Used with `eval-after-load'."
  (let* ((version (let ((s (shell-command-to-string (concat gd-python-command
                                " -V"))))
...
(defun gd-help-at-point (&optional debug)
  ...
  (let* ((orig (point))
    ...
    (if erg
      ...
      (with-temp-buffer
        (insert cmd)
        (write-file file))
      (setq erg (gd-process-file file "*Python-Help*"))
      (if gd-max-help-buffer-p
          (progn
          ...
          (setq funcdoc (documentation func)
                keys (concat
                      ...
                      (mapconcat 'key-description
                                 (where-is-internal func godot-mode-map)
                                 ", "))))
...
(defun gd-describe-mode ()
  "Dump long form of `godot-mode' docs."
  (interactive)
  (gd--dump-help-string "Major mode for editing Python files.
...
\\[gd-execute-import-or-reload]\timports or reloads the file in the Python interpreter
\\[gd-execute-buffer]\tsends the entire buffer to the Python interpreter
...
\\[gd-shell]\tstarts a Python interpreter window; this will be used by
...
gd-install-directory\twherefrom `godot-mode' looks for extensions
gd-indent-offset\tindentation increment
gd-block-comment-prefix\tcomment string used by comment-region

gd-shell-name\tshell command to invoke Python interpreter
...
The `godot-mode' commands generally work on statements instead of on
...
automatically by `godot-mode' is just an educated guess:  only you know
...
\t c = d
\t _
`godot-mode' cannot know whether that's what you intended, or whether
\tif a > 0:
\t c = d
\t_
was your intent.  In general, `godot-mode' either reproduces the
...
`godot-mode-hook', if that value exists and is not nil; for backward
compatibility it also tries `gd-mode-hook'; see the `Hooks' section of
the Elisp manual for details.

Obscure:  When godot-mode is first loaded, it looks for all bindings
...
Interactively, prompt for SYMBOL."
  ...
  (let* ((last-window-configuration
    ...
    (if local
      ...
      (setq source (gd--send-string-return-output (concat imports "import inspect;inspect.getmodule(" symbol ")")))
      (cond ((string-match "SyntaxError" source)
            ...
            ((and source (string-match "builtin" source))
             (progn
               ...
               ;; (jump-to-register 98888888)
                    (message "%s" source)))
            ((and source (setq path (replace-regexp-in-string "'" "" (gd--send-string-return-output "import os;os.getcwd()")))
                  ...
                  (find-file (concat path (char-to-string gd-separator-char) sourcefile))
                  (goto-char (point-min))
                  (re-search-forward (concat gd-def-or-class-re symbol) nil nil 1))
             (push-mark)
             (goto-char (match-beginning 0))
             (exchange-point-and-mark)
             (display-buffer gd-exception-buffer)))
...
Home-page: http://www.logilab.org/project/pylint "
  (interactive
   (let ((default
           (if (buffer-file-name)
               (format "%s %s %s" gd-pylint-command
...
(eval-after-load "ffap"
  '(progn
     (push '(godot-mode . gd-ffap-module-path) ffap-alist)
...
(defun variables-state (&optional buffer directory-in directory-out)
  "Diplays state of godot-mode variables in an org-mode buffer.

Reads variables from godot-mode.el as current buffer.
...
(defun variables-base-state (gd-exception-buffer orgname reSTname directory-in directory-out)
  (save-restriction
    (let ((suffix (file-name-nondirectory (buffer-file-name)))
      ...
      ;; (with-temp-buffer
      (set-buffer (get-buffer-create "State-of-godot-mode-variables.org"))
      (erase-buffer)
      ;; org
      (insert "State of godot-mode variables\n\n")
      (switch-to-buffer (current-buffer))
...
(defun gd-match-paren-mode (&optional arg)
  ...
  (if (or arg (not gd-match-paren-mode))
    ...
    (setq gd-match-paren-mode t)
        ;;  (define-key godot-mode-map (kbd (concat "<" gd-match-paren-key ">")) 'gd-match-paren))
...
With optional \\[universal-argument] print as string"
  ...
  (let* ((name (gd--string-strip (or arg (car kill-ring))))
         ...
         (form (cond ((or (eq major-mode 'godot-mode)(eq major-mode 'gd-shell-mode))
                      (if numbered
...
(defun gd-switch-imenu-index-function ()
  ...
  (if (eq major-mode 'godot-mode)
    ...
    (error "%s" "Only available in buffers set to godot-mode")))
  ...
  (let* ((orig (point))
     (erg
      (cond ((gd--end-of-statement-p)
         (and (gd-forward-statement) (gd-backward-statement)))
        ((ignore-errors (< orig (progn (gd-forward-statement) (gd-backward-statement))))
         (point))
...
(defun gd-execute-block-python-switch ()
  "Send block at point to default interpreter.
...
(defun gd-execute-block-python-no-switch ()
  "Send block at point to default interpreter.
...
(defun gd-execute-block-python-dedicated ()
...
(defun gd-execute-block-python-dedicated-switch ()
...
(defun gd-execute-block-or-clause-python-switch ()
  "Send block-or-clause at point to default interpreter.
...
(defun gd-execute-block-or-clause-python-no-switch ()
  "Send block-or-clause at point to default interpreter.
...
(defun gd-execute-block-or-clause-python-dedicated ()
...
(defun gd-execute-block-or-clause-python-dedicated-switch ()
...
(defun gd-execute-buffer-python-switch ()
  "Send buffer at point to default interpreter.
...
(defun gd-execute-buffer-python-no-switch ()
  "Send buffer at point to default interpreter.
...
(defun gd-execute-buffer-python-dedicated ()
...
(defun gd-execute-buffer-python-dedicated-switch ()
...
(defun gd-execute-class-python-switch ()
  "Send class at point to default interpreter.
...
(defun gd-execute-class-python-no-switch ()
  "Send class at point to default interpreter.
...
(defun gd-execute-class-python-dedicated ()
...
(defun gd-execute-class-python-dedicated-switch ()
...
(defun gd-execute-clause-python-switch ()
  "Send clause at point to default interpreter.
...
(defun gd-execute-clause-python-no-switch ()
  "Send clause at point to default interpreter.
...
(defun gd-execute-clause-python-dedicated ()
...
(defun gd-execute-clause-python-dedicated-switch ()
...
(defun gd-execute-def-python-switch ()
  "Send def at point to default interpreter.
...
(defun gd-execute-def-python-no-switch ()
  "Send def at point to default interpreter.
...
(defun gd-execute-def-python-dedicated ()
...
(defun gd-execute-def-python-dedicated-switch ()
...
(defun gd-execute-def-or-class-python-switch ()
  "Send def-or-class at point to default interpreter.
...
(defun gd-execute-def-or-class-python-no-switch ()
  "Send def-or-class at point to default interpreter.
...
(defun gd-execute-def-or-class-python-dedicated ()
...
(defun gd-execute-def-or-class-python-dedicated-switch ()
...
(defun gd-execute-expression-python-switch ()
  "Send expression at point to default interpreter.
...
(defun gd-execute-expression-python-no-switch ()
  "Send expression at point to default interpreter.
...
(defun gd-execute-expression-python-dedicated ()
...
(defun gd-execute-expression-python-dedicated-switch ()
...
(defun gd-execute-line-python-switch ()
  "Send line at point to default interpreter.
...
(defun gd-execute-line-python-no-switch ()
  "Send line at point to default interpreter.
...
(defun gd-execute-line-python-dedicated ()
...
(defun gd-execute-line-python-dedicated-switch ()
...
(defun gd-execute-minor-block-python-switch ()
  "Send minor-block at point to default interpreter.
...
(defun gd-execute-minor-block-python-no-switch ()
  "Send minor-block at point to default interpreter.
...
(defun gd-execute-minor-block-python-dedicated ()
...
(defun gd-execute-minor-block-python-dedicated-switch ()
...
(defun gd-execute-paragraph-python-switch ()
  "Send paragraph at point to default interpreter.
...
(defun gd-execute-paragraph-python-no-switch ()
  "Send paragraph at point to default interpreter.
...
(defun gd-execute-paragraph-python-dedicated ()
...
(defun gd-execute-paragraph-python-dedicated-switch ()
...
(defun gd-execute-partial-expression-python-switch ()
  "Send partial-expression at point to default interpreter.
...
(defun gd-execute-partial-expression-python-no-switch ()
  "Send partial-expression at point to default interpreter.
...
(defun gd-execute-partial-expression-python-dedicated ()
...
(defun gd-execute-partial-expression-python-dedicated-switch ()
...
(defun gd-execute-region-python-switch (beg end)
  "Send region at point to default interpreter.
...
(defun gd-execute-region-python-no-switch (beg end)
  "Send region at point to default interpreter.
...
(defun gd-execute-region-python-dedicated (beg end)
...
(defun gd-execute-region-python-dedicated-switch (beg end)
...
(defun gd-execute-statement-python-switch ()
  "Send statement at point to default interpreter.
...
(defun gd-execute-statement-python-no-switch ()
  "Send statement at point to default interpreter.
...
(defun gd-execute-statement-python-dedicated ()
...
(defun gd-execute-statement-python-dedicated-switch ()
...
(defun gd-execute-top-level-python-switch ()
  "Send top-level at point to default interpreter.
...
(defun gd-execute-top-level-python-no-switch ()
  "Send top-level at point to default interpreter.
...
(defun gd-execute-top-level-python-dedicated ()
...
(defun gd-execute-top-level-python-dedicated-switch ()
...
(defun gd-edit-abbrevs ()
  "Jumps to `godot-mode-abbrev-table' in a buffer containing lists of abbrev definitions.
...
(defun gd-add-abbrev (arg)
  "Defines godot-mode specific abbrev for last expressions before point.
  ...
  (let* ((thisbeg (copy-marker docstring))
    ...
    (cond ((string-match (concat "^" gd-labelled-re) (buffer-substring-no-properties beg end))
          ...
          ((save-excursion (goto-char end)
           ...
           (gd--fill-docstring-last-line thisbeg thisend beg end style orig first-line-p gd-current-indent docstring))
...
(defun gd--shift-forms-base (form arg &optional beg end)
  (let* ((begform (intern-soft (concat "gd-beginning-of-" form)))
         (endform (intern-soft (concat "gd-end-of-" form)))
...
(defun gd-shift-line-right (&optional arg)
...
Returns outmost indentation reached. "
  (interactive "*P")
  (let ((erg (gd--shift-forms-base "line" (or arg gd-indent-offset))))
        (when (called-interactively-p 'any) (message "%s" erg))
    erg))

(defun gd-shift-line-left (&optional arg)
...
(defun gd-execute-file-python-switch (&optional filename)
  "Send file to Python default interpreter.
...
(defun gd-execute-file-python-no-switch (&optional filename)
  "Send file to Python default interpreter.
...
(defun gd-execute-file-python-dedicated (&optional filename)
  "Send file to Python default interpreter.
...
(defun gd-execute-file-python-dedicated-switch (&optional filename)
  "Send file to Python default interpreter.
...
(defun gd-backward-comment (&optional last orig)
  ...
  (unless (bobp)
    (let ((pps (parse-partial-sexp (point-min) (point)))
      ...
      (progn
        ...
        (setq last (point))
        (skip-chars-backward " \t\r\n\f")
        (gd-backward-comment last orig))
    (when last (goto-char last)))
      (when (< (point) orig) (point)))))

(defun gd-beginning-of-comment ()
  "Go to the beginning of current line's comment, if any.
...
the default"
  ...
  (save-excursion
    (let ((comment-start (if gd-block-comment-prefix-p
          ...
          (end (or end (gd--end-of-block-position))))
      (goto-char beg)
      (push-mark)
      (goto-char end)
      (comment-region beg end arg))))
...
the default"
  ...
  (save-excursion
    (let ((comment-start (if gd-block-comment-prefix-p
          ...
          (end (or end (gd--end-of-minor-block-position))))
      (goto-char beg)
      (push-mark)
      (goto-char end)
      (comment-region beg end arg))))
...
the default"
  ...
  (save-excursion
    (let ((comment-start (if gd-block-comment-prefix-p
          ...
          (end (or end (gd--end-of-top-level-position))))
      (goto-char beg)
      (push-mark)
      (goto-char end)
      (comment-region beg end arg))))
...
the default"
  ...
  (save-excursion
    (let ((comment-start (if gd-block-comment-prefix-p
          ...
          (end (or end (gd--end-of-clause-position))))
      (goto-char beg)
      (push-mark)
      (goto-char end)
      (comment-region beg end arg))))
...
the default"
  ...
  (save-excursion
    (let ((comment-start (if gd-block-comment-prefix-p
          ...
          (end (or end (gd--end-of-block-or-clause-position))))
      (goto-char beg)
      (push-mark)
      (goto-char end)
      (comment-region beg end arg))))
...
the default"
  ...
  (save-excursion
    (let ((comment-start (if gd-block-comment-prefix-p
          ...
          (end (or end (gd--end-of-def-position))))
      (goto-char beg)
      (push-mark)
      (goto-char end)
      (comment-region beg end arg))))
...
the default"
  ...
  (save-excursion
    (let ((comment-start (if gd-block-comment-prefix-p
          ...
          (end (or end (gd--end-of-class-position))))
      (goto-char beg)
      (push-mark)
      (goto-char end)
      (comment-region beg end arg))))
...
the default"
  ...
  (save-excursion
    (let ((comment-start (if gd-block-comment-prefix-p
          ...
          (end (or end (gd--end-of-def-or-class-position))))
      (goto-char beg)
      (push-mark)
      (goto-char end)
      (comment-region beg end arg))))
...
the default"
  ...
  (save-excursion
    (let ((comment-start (if gd-block-comment-prefix-p
          ...
          (end (or end (gd--end-of-statement-position))))
      (goto-char beg)
      (push-mark)
      (goto-char end)
      (comment-region beg end arg))))
...
(defun gd-fast-process (&optional argprompt dedicated shell buffer-name)
...
Return the process"
  (interactive "P")
  (gd-shell argprompt dedicated shell buffer-name t))
...
;;     (setq gd-result (replace-regexp-in-string (format "[ \n]*%s[ \n]*" gd-fast-filter-re) "" erg))
;;     gd-result))
...
(defun gd--fast-send-string (string &optional proc windows-config)
...
"
  (let* ((proc (or proc (get-buffer-process (gd-fast-process))))
     (buffer (process-buffer proc)))
    (if (or gd-store-result-p gd-return-result-p)
    (gd--fast-send-string-intern string proc buffer gd-store-result-p gd-return-result-p)
      (gd--fast-send-string-no-output string proc buffer))))

(defun gd-execute-string-fast (string)
  "Evaluate STRING in Python process which is not in comint-mode.

From a programm use `gd--fast-send-string'"
  (interactive "sPython command: ")
  (gd--fast-send-string string))
...
;; (setq hs-forward-sexp-func 'gd-end-of-block)
...
(defvaralias 'gd-mode-map 'godot-mode-map)

(defvar gd-python-shell-mode-map
  (let ((map (make-sparse-keymap)))
...
(defvar gd-ipython-shell-mode-map gd-python-shell-mode-map
  "Unless setting of ipython-shell-mode needs to be different, let's save some lines of code and copy gd-python-shell-mode-map here.")

(defvar gd-shell-map gd-python-shell-mode-map)

(when gd-org-cycle-p
  (define-key godot-mode-map (kbd "<backtab>") 'org-cycle))
...
(defun gd-load-skeletons ()
  ...
  (interactive)
  (load (concat gd-install-directory "/extensions/python-components-skeletons.el")))
...
;;  sys.version_info[0]
(defun gd-python-version (&optional executable verbose)
...
(defun gd-version ()
  "Echo the current version of `godot-mode' in the minibuffer."
  (interactive)
  (message "Using `godot-mode' version %s" gd-version)
...
(when gd--warn-tmp-files-left-p
  (add-hook 'godot-mode-hook 'gd--warn-tmp-files-left))
...
\\[universal-argument] `gd-toggle-shell' prompts to specify a reachable Python command.
\\[universal-argument] followed by numerical arg 2 or 3, `gd-toggle-shell' opens a respective Python shell.
...
Should you need more shells to select, extend this command by adding inside the first cond:
  ...
  (let ((name (cond ((eq 2 (prefix-numeric-value arg))
                    ...
                    ((eq 4 (prefix-numeric-value arg))
                     (gd--string-strip
                      (read-from-minibuffer "Python Shell: " gd-shell-name) "\" " "\" "
                      ))
...
(make-obsolete 'jgodot-mode 'jython-mode nil)

(add-to-list 'same-window-buffer-names (purecopy "*Python*"))
(add-to-list 'same-window-buffer-names (purecopy "*IPython*"))

(add-to-list 'auto-mode-alist (cons (purecopy "\\.gd\\'")  'godot-mode))

;;  (add-to-list 'interpreter-mode-alist
;;  (cons (purecopy "[bi]*python[0-9.]*") 'godot-mode))
;;
;;  (add-to-list 'interpreter-mode-alist
;;  (cons (purecopy "jython[0-9.]*") 'jython-mode))

(add-to-list 'magic-mode-alist
         '("!#[ \t]*/.*[jp]ython[0-9.]*" . godot-mode))
...
(defvar gd--timer nil
  "Used by `gd--run-unfontify-timer'")
(make-variable-buffer-local 'gd--timer)

(defvar gd--timer-delay nil
  "Used by `gd--run-unfontify-timer'")
(make-variable-buffer-local 'gd--timer-delay)

(defun gd--unfontify-banner-intern (buffer)
  (save-excursion
    (goto-char (point-min))
    (let ((erg (or (ignore-errors (car comint-last-prompt))
      ...
      (progn
        ...
        (goto-char (point-max)))
    (progn (and gd-debug-p (message "%s" (concat "gd--unfontify-banner: Don't see a prompt in buffer " (buffer-name buffer)))))))))

(defun gd--unfontify-banner (&optional buffer)
  "Unfontify the shell banner-text.

Cancels `gd--timer'
Expects being called by `gd--run-unfontify-timer' "
  (interactive)
    (let ((buffer (or buffer (current-buffer))))
      (if (ignore-errors (buffer-live-p (get-buffer buffer)))
      (with-current-buffer buffer
        (gd--unfontify-banner-intern buffer)
        (and (timerp gd--timer)(cancel-timer gd--timer)))
    (and (timerp gd--timer)(cancel-timer gd--timer)))))

(defun gd--run-unfontify-timer (&optional buffer)
  "Unfontify the shell banner-text "
  (when gd--shell-unfontify
    (let ((buffer (or buffer (current-buffer)))
      ...
      (if (and
       ...
       (or
        (eq major-mode 'gd-python-shell-mode)
        (eq major-mode 'gd-ipython-shell-mode)))
      (unless gd--timer
        (setq gd--timer
          (run-with-idle-timer
           (if gd--timer-delay (setq gd--timer-delay 3)
             (setq gd--timer-delay 0.1))
           nil
           #'gd--unfontify-banner buffer)))
    (cancel-timer gd--timer)))))
...
(remove-hook 'godot-mode-hook 'python-setup-brm)
;; ;
...
Use `defcustom' to keep value across sessions "
  (interactive
   (list
    (read-from-minibuffer "Command args: " gd-python-command-args)))
    (setq gd-python-command-args arguments))

(defun gd---emacs-version-greater-23 ()
  "Return `t' if emacs major version is above 23"
  (< 23 (string-to-number (car (split-string emacs-version "\\.")))))

;; (defun gd-backward-comment (&optional last)
        ...
        (setq indent
          (cond
           ((and (bobp) (eq liep (line-end-position)))
            0)
           ((and (bobp)(gd--statement-opens-block-p gd-extended-block-or-clause-re))
            (+ (if gd-smart-indentation (gd-guess-indent-offset) indent-offset) (current-indentation)))
           ((and (bobp)(not (gd--statement-opens-block-p gd-extended-block-or-clause-re)))
            (current-indentation))
           ;; in string
           ...
           ((nth 1 pps)
            ...
            (save-excursion
              ...
              (progn
                (gd-backward-statement)
                (gd-compute-indentation orig origline closing line nesting repeat indent-offset liep))
            (goto-char (+ gd-lhs-inbound-indent (nth 1 pps)))
            (when (looking-at "[ \t]+")
              (goto-char (match-end 0)))
            (current-column))))
  ...
  (save-restriction
    ...
    (save-excursion
      (let ((erg (when (gd-backward-def-or-class)
        ...
        (when (and erg gd-current-defun-show (push-mark (point) t t) (skip-chars-forward "^ (")
                   (exchange-point-and-mark)
...
(defun gd-which-function ()
  ...
  (save-excursion
    (save-restriction
      (widen)
      (let ((orig (point))
            (erg (if (and (looking-at (concat gd-def-or-class-re " +\\([^(]+\\)(.+")) (not (gd-in-string-or-comment-p)))
                     (match-string-no-properties 2)
                   (progn
                     (gd-backward-def-or-class)
                     (when (looking-at (concat gd-def-or-class-re " +\\([^(]+\\)(.+"))
                       (match-string-no-properties 2))))))
        (if (and erg (< orig (gd-forward-def-or-class)))
            (when (called-interactively-p 'any) (message "%s" erg))
          (setq erg nil)
          (when (called-interactively-p 'any) (message "%s" "Not inside a function or class"))
          erg)))))

(defconst gd-help-address "godot-mode@python.org"
  "List dealing with usage and developing godot-mode.

Also accepts submission of bug reports, whilst a ticket at
http://launchpad.net/godot-mode
...
;;  (defun gd-shell-send-string (string &optional process msg filename)
;;    "Send STRING to Python PROCESS.
;;  When `gd-verbose-p' and MSG is non-nil messages the first line of STRING."
;;    (interactive "sPython command: ")
;;    (let* ((process (or process (get-buffer-process (gd-shell))))
;;           (lines (split-string string "\n"))
;;           (temp-file-name (concat (with-current-buffer (process-buffer process)
;;                                     (file-remote-p default-directory))
;;                                   (gd--normalize-directory gd-temp-directory)
;;                   ;; (md5 (user-login-name))
...
;;        (insert string)
;;        (delete-trailing-whitespace)
;;        (gd-send-file temp-file-name process temp-file-name))
;;        (comint-send-string process string)
;;        (when (or (not (string-match "\n$" string))
;;                  (string-match "\n[ \t].*\n?$" string))
;;          (comint-send-string process "\n")))
;;      (unless gd-debug-p (when (file-readable-p temp-file-name)(delete-file temp-file-name)))))

(defun gd--delay-process-dependent (process)
  "Call a `gd-ipython-send-delay' or `gd-python-send-delay' according to process"
  (if (string-match "ipython" (prin1-to-string process))
      (sit-for gd-ipython-send-delay t)
    (sit-for gd-python-send-delay t)))
...
(defun gd--backward-prepare (indent final-re &optional inter-re iact lc)
  (let ((orig (point))
        (indent
         (or indent
         (cond ((looking-back "^[ \t]*")
...
(defun gd--fetch-first-python-buffer ()
...
(defun gd-unload-python-el ()
  "Unloads godot-mode delivered by shipped python.el
...
Returns position if successful, nil otherwise "
  ...
  (let (erg)
    (unless (bobp)
      (while (and (not (bobp)) (setq erg (gd-backward-statement))
...
If GD-MARK-DECORATORS, `def'- and `class'-forms include decorators
...
(defun gd--forms-report-result (erg)
...
(and (ignore-errors (require 'easymenu) t)
     ;; (easy-menu-define gd-menu map "Python Tools"
     ;;           `("PyTools"
     (easy-menu-define
       gd-menu godot-mode-map "Python Mode menu"
       `("Python"
     ("Interpreter"
          ["Ipython" ipython
       :help " `ipython'
...
Mark try-block at point."]
          ...
          ("Copy"
       ["Copy block" gd-cogd-block
        :help " `gd-cogd-block'
Copy block at point."]

       ["Copy block or clause" gd-cogd-block-or-clause
        :help " `gd-cogd-block-or-clause'
Copy block-or-clause at point."]

       ["Copy class" gd-cogd-class
        :help " `gd-cogd-class'
Copy class at point."]

       ["Copy clause" gd-cogd-clause
        :help " `gd-cogd-clause'
Copy clause at point."]

       ["Copy comment" gd-cogd-comment
        :help " `gd-cogd-comment'"]

       ["Copy def" gd-cogd-def
        :help " `gd-cogd-def'
Copy def at point."]

       ["Copy def or class" gd-cogd-def-or-class
        :help " `gd-cogd-def-or-class'
Copy def-or-class at point."]

       ["Copy except block" gd-cogd-except-block
        :help " `gd-cogd-except-block'"]

       ["Copy expression" gd-cogd-expression
        :help " `gd-cogd-expression'
Copy expression at point."]

       ["Copy if block" gd-cogd-if-block
        :help " `gd-cogd-if-block'"]

       ["Copy line" gd-cogd-line
        :help " `gd-cogd-line'
Copy line at point."]

       ["Copy minor block" gd-cogd-minor-block
        :help " `gd-cogd-minor-block'
Copy minor-block at point."]

       ["Copy paragraph" gd-cogd-paragraph
        :help " `gd-cogd-paragraph'
Copy paragraph at point."]

       ["Copy partial expression" gd-cogd-partial-expression
        :help " `gd-cogd-partial-expression'
Copy partial-expression at point."]

       ["Copy section" gd-cogd-section
        :help " `gd-cogd-section'"]

       ["Copy statement" gd-cogd-statement
        :help " `gd-cogd-statement'
Copy statement at point."]

       ["Copy top level" gd-cogd-top-level
        :help " `gd-cogd-top-level'
Copy top-level at point."]

       ["Copy try block" gd-cogd-try-block
        :help " `gd-cogd-try-block'"]
           )
          ("Kill"
...
Display flake8 command line help messages."]
      ...
      ["godot-mode customize group" (customize-group 'godot-mode)
...
Otherwise value of gd-python-history is used. Use `M-x customize-variable' to set it permanently"
        ...
        ["Enforce local Python shell " gd-force-local-shell-on
         :help "Locally indicated Python being enforced upon sessions execute commands. "]

        ["Remove local Python shell enforcement, restore default" gd-force-local-shell-off
...
Output-buffer is not in comint-mode"
        ...
        ["Python mode v5 behavior"
         (setq godot-mode-v5-behavior-p
           (not godot-mode-v5-behavior-p))
         :help "Execute region through `shell-command-on-region' as
v5 did it - lp:990079. This might fail with certain chars - see UnicodeEncodeError lp:550661

Use `M-x customize-variable' to set it permanently"
         :style toggle :selected godot-mode-v5-behavior-p]

        ["Force shell name "
         (setq gd-force-gd-shell-name-p
           (not gd-force-gd-shell-name-p))
         :help "When `t', execution with kind of Python specified in `gd-shell-name' is enforced, possibly shebang doesn't take precedence. Use `M-x customize-variable' to set it permanently"
...
In experimental state yet "
        ...
        ["Warn tmp files left "
         ...
         :help "Messages a warning, when `gd-temp-directory' contains files susceptible being left by previous godot-mode sessions. See also lp:987534 Use `M-x customize-variable' to set it permanently"
       ...
       ("Edit"
        ...
        ("Completion"
         ...
         ["Set Pymacs-based complete keymap "
          ...
          :help "If `gd-complete-initialize', which sets up enviroment for Pymacs based gd-complete, should load it's keys into `godot-mode-map'
...
Use `M-x customize-variable' to set it permanently"
          ...
          ["Tab indent"
           (setq gd-tab-indent
             (not gd-tab-indent))
           :help "Non-nil means TAB in Python mode calls `gd-indent-line'.Use `M-x customize-variable' to set it permanently"
...
In result cursor is insided emptied delimited form.Use `M-x customize-variable' to set it permanently"
         ...
         ["Trailing whitespace smart delete "
          ...
          :help "Default is nil. When t, godot-mode calls
    (add-hook 'before-save-hook 'delete-trailing-whitespace nil 'local)
...
Use `M-x customize-variable' to set it permanently"
        ...
        ["Keep windows configuration"
         ...
         :help "If a windows is splitted displaying results, this is directed by variable `gd-split-window-on-execute'\. Also setting `gd-switch-buffers-on-execute-p' affects window-configuration\. While commonly a screen splitted into source and Python-shell buffer is assumed, user may want to keep a different config\.
...
Default is nil. In help-buffer, \"q\" will close it.  Use `M-x customize-variable' to set it permanently"
        ...
        ["Current defun show"
         ...
         :help "If `gd-current-defun' should jump to the definition, highlight it while waiting GD-WHICH-FUNC-DELAY seconds, before returning to previous position.
...
Making switch between several virtualenv's easier,
                               `godot-mode' should deliver an installer, so named-shells pointing to virtualenv's will be available. Use `M-x customize-variable' to set it permanently"
          :style toggle :selected gd-use-local-default]

         ["Use current dir when execute "
          (setq gd-use-current-dir-when-execute-p
            (not gd-use-current-dir-when-execute-p))
          :help "When `t', current directory is used by Python-shell for output of `gd-execute-buffer' and related commands.
...
See also `gd-execute-directory'Use `M-x customize-variable' to set it permanently"
         ...
         ["Fileless buffer use default directory "
          ...
          :help "When `gd-use-current-dir-when-execute-p' is non-nil and no buffer-file exists, value of `default-directory' sets current working directory of Python output shellUse `M-x customize-variable' to set it permanently"
...
set it permanently"
        ...
        ["Edit only "
         (setq gd-edit-only-p
           (not gd-edit-only-p))
         :help "When `t' `godot-mode' will not take resort nor check for installed Python executables. Default is nil.
...
Delete the following character or all following whitespace"]
          ...
          ("Filling"
       ["Py docstring style" gd-docstring-style
        :help " `gd-docstring-style'"]

       ["Py fill comment" gd-fill-comment
        :help " `gd-fill-comment'"]

       ["Py fill paragraph" gd-fill-paragraph
        :help " `gd-fill-paragraph'"]

       ["Py fill string" gd-fill-string
        :help " `gd-fill-string'"]

       ["Py fill string django" gd-fill-string-django
        :help " `gd-fill-string-django'"]

       ["Py fill string onetwo" gd-fill-string-onetwo
        :help " `gd-fill-string-onetwo'"]

       ["Py fill string pep 257" gd-fill-string-pep-257
        :help " `gd-fill-string-pep-257'"]

       ["Py fill string pep 257 nn" gd-fill-string-pep-257-nn
        :help " `gd-fill-string-pep-257-nn'"]

       ["Py fill string symmetric" gd-fill-string-symmetric
        :help " `gd-fill-string-symmetric'"]
            )
          ("Abbrevs"       :help "see also `gd-add-abbrev'"
       :filter (lambda (&rest junk)
             (abbrev-table-menu godot-mode-abbrev-table))            )

          ["Add abbrev" gd-add-abbrev
       :help " `gd-add-abbrev'
Defines godot-mode specific abbrev for last expressions before point."]
          ("Completion"
       ["Py indent or complete" gd-indent-or-complete
        :help " `gd-indent-or-complete'"]

       ["Py shell complete" gd-shell-complete
        :help " `gd-shell-complete'"]

       ["Py complete" gd-complete
        :help " `gd-complete'"]
            )
...
(and (ignore-errors (require 'easymenu) t)
     ...
     ;;           `("PyTools"
     (easy-menu-define
       gd-shell-menu gd-python-shell-mode-map "Gd-Shell Mode menu"
       `("Gd-Shell"
         ("Edit"
          ("Shift"
           ("Shift right"
...
(define-derived-mode gd-auto-completion-mode godot-mode "Pac"
  "Run auto-completion"
  ;; disable company
  ;; (when company-mode (company-mode))
  (if gd-auto-completion-mode-p
...
;; (add-hook 'after-change-major-mode-hook #'gd-protect-other-buffers-ac)

;; after-change-major-mode-hook

;;;
(define-derived-mode godot-mode fundamental-mode godot-mode-modeline-display
  "Major mode for editing Python files.

To submit a problem report, enter `\\[gd-submit-bug-report]' from a
`godot-mode' buffer.  Do `\\[gd-describe-mode]' for detailed
documentation.  To see what version of `godot-mode' you are running,
enter `\\[gd-version]'.
...
`gd-shell'\tStart an interactive Python interpreter in another window
`gd-execute-statement'\tSend statement at point to Python default interpreter
`gd-backward-statement'\tGo to the initial line of a simple statement

etc.

See available commands listed in files commands-godot-mode at directory doc
...
`gd-switch-buffers-on-execute-p'    When non-nil switch to the Python output buffer

See available customizations listed in files variables-godot-mode at directory doc
...
\\{godot-mode-map}"
  ...
  (cond ((string-match "ython3" gd-python-edit-version)
     (font-lock-add-keywords 'godot-mode
                 '(("\\<print\\>" . 'gd-builtins-face)
                   ("\\<file\\>" . nil))))
    (t (font-lock-add-keywords 'godot-mode
  ...
  ;; (add-hook 'godot-mode-hook 'gd--run-completion-timer)
  ;; (remove-hook 'godot-mode-hook 'gd--run-completion-timer))
  ;; (when gd-auto-complete-p
  ;; (add-hook 'godot-mode-hook
  ;; (lambda ()
  ;; (run-with-idle-timer 1 t 'gd-shell-complete))))
  (if gd-auto-fill-mode
      (add-hook 'godot-mode-hook 'gd--run-auto-fill-timer)
    (remove-hook 'godot-mode-hook 'gd--run-auto-fill-timer))
  ...
  (if gd-defun-use-top-level-p
      (progn
        ...
        (define-key godot-mode-map [(control meta a)] 'gd-backward-top-level)
        (define-key godot-mode-map [(control meta e)] 'gd-end-of-top-level))
    ...
    (define-key godot-mode-map [(control meta a)] 'gd-backward-def-or-class)
    (define-key godot-mode-map [(control meta e)] 'gd-end-of-def-or-class))
  (when gd-sexp-use-expression-p
        (define-key godot-mode-map [(control meta f)] 'gd-forward-expression)
    (define-key godot-mode-map [(control meta b)] 'gd-backward-expression))
  ...
  (when (called-interactively-p 'any) (message "godot-mode loaded from: %s" godot-mode-message-string))
  (force-mode-line-update))

(defun gd--all-shell-mode-setting ()
  (when gd-fontify-shell-buffer-p
    (set (make-local-variable 'font-lock-defaults)
     '(python-font-lock-keywords nil nil nil nil
                     (font-lock-syntactic-keywords
                      . gd-font-lock-syntactic-keywords))))
  ...
  ;; It might be useful having a different setting of `comint-use-prompt-regexp' in gd-shell - please report when a use-case shows up
...
(define-derived-mode gd-python-shell-mode comint-mode "Py"
  "Major mode for interacting with a Python process.
A Python process can be started with \\[gd-shell].

You can send text to the Python process from other buffers
containing Python source.
 * \\[gd-execute-region] sends the current region to the Python process.

Sets basic comint variables, see also versions-related stuff in `gd-shell'.
\\{gd-python-shell-mode-map}"
  :group 'godot-mode
  ;; (require 'ansi-color) ; for ipython
  (setq mode-line-process '(":%s"))
  ;; (sit-for 0.1)
  (when gd-verbose-p (message "%s" "Initializing Python shell, please wait"))
  (gd--all-shell-mode-setting)
  (gd--python-send-completion-setup-code)
  (gd--python-send-ffap-setup-code)
  (gd--python-send-eldoc-setup-code)
  ;; (if gd-shell-unfontify-p
  ;; (add-hook 'gd-python-shell-mode-hook #'gd--run-unfontify-timer (current-buffer))
  ;; (remove-hook 'gd-python-shell-mode-hook 'gd--run-unfontify-timer))
  ...
  (if gd-complete-function
    ...
    (add-to-list (make-local-variable 'comint-dynamic-complete-functions)
         'gd-shell-complete))
  (when gd-sexp-use-expression-p
    (define-key gd-python-shell-mode-map [(control meta f)] 'gd-forward-expression)
    (define-key gd-python-shell-mode-map [(control meta b)] 'gd-backward-expression))
  (when gd-shell-menu
    (easy-menu-add gd-menu))
  (if gd-shell-unfontify-p
      (add-hook 'gd-python-shell-mode-hook #'gd--run-unfontify-timer (current-buffer))
...
\\{gd-ipython-shell-mode-map}"
  ...
  (sit-for 0.1 t)
  ;; (gd--unfontify-banner-intern)
  (if gd-shell-unfontify-p
      (add-hook 'gd-ipython-shell-mode-hook #'gd--run-unfontify-timer (current-buffer))
    (remove-hook 'gd-ipython-shell-mode-hook 'gd--run-unfontify-timer))
